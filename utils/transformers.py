...
class BlogsPagesTransformer(BaseTransformer):
    ...
    def restructure(self):
        ...
        page['fields'] = {
            "title": a['title'],
            "slug": slugify(a['title']),
            "url_path": "/index/blogs/{}/".format(slugify(a['title'])),
            "live": True,
            "seo_title": a['title'],
            "depth": 4,
            "content_type_id": 37,
            "path": "000100020001000{}".format(page['pk']),
        }
        self.transformable = page
...
class MaltaTransformer(ArticlesTransformer):
    ...
    def restructure(self):
        ...
        a['fields']['date'] = a['fields']['date'].strftime('%Y-%m-%d')
        a['fields']['content'] = a['fields']['content'].replace('\r', '').replace('\n', '')
        a['fields']['body'] = body % (json.dumps(a['fields']['content']), uuid.uuid4()),
        a['fields']['body'] = a['fields']['body'][0].replace('\\\\', '\\')
        a['pk'] = a['fields']['pk']
...
class MaltaPagesTransformer(BaseTransformer):
